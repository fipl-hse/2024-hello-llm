...
class RawDataPreprocessor(AbstractRawDataPreprocessor):
    ...
    def transform(self) -> None:
        ...
        self._data[ColumnNames.TARGET.value] = (
            self._data[ColumnNames.TARGET.value].apply(lambda x: label_map[x]))
...
class LLMPipeline(AbstractLLMPipeline):
    ...
    def analyze_model(self) -> dict:
        ...
        input_data = {"attention_mask": attention_mask, "input_ids": input_ids}
    ...
    def _infer_batch(self, sample_batch: Sequence[tuple[str, ...]]) -> list[str]:
        ...
        decoded_preds = self._model(**inputs).logits
        return [str(i) for i in torch.argmax(decoded_preds, dim=1).tolist()]
